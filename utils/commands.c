...
void help() {
    ...
    printf(reset);
    printf(CYN);
    puts("\tpandash_send [THE_OTHER_SHELL_ID] [YOUR QOUTED MSG]: (ID can be "
         "anything)");
    printf(reset);
    printf(BRED);
    puts("\t\tTRYS TO DELIEVER YOUR MSG, YOUR ID IS PRINTED AS SOON AS YOU "
...
void get_directory(char *path) { getcwd(path, 512); }

void exit_pandash() { exit(0); }
